...
fn export_game_context<R: TauriRuntime>(
    ...
    // mod 目录：**用户不给就表示不加载 mod**（我们给游戏写了自己的 `config.ini`，游戏不会
    // 去它的用户数据目录找 mod，只会读原版 + DLC 内容）——所以这里**不推导、不猜**
    // `<游戏>/mods`。给了就显式传给游戏，保证「dump 里加载的 mod」与「我们解析图标、记
    // 元数据用的目录」是同一个。
    let mod_dir: Option<PathBuf> = mod_dir.map(PathBuf::from);
    if let Some(dir) = &mod_dir {
        eprintln!("本次导出使用 mod 目录：{}", dir.display());
    }
