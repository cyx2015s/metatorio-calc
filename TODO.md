...
- **暂缓（2026-xx 用户决定）** 无头模式：不启用 GUI 跑 MCP。调研结论——Tauri 在 Linux 上即使不建窗口，启动时仍会初始化 GTK/X11（群里用 xvfb 正是为此），所以「同一二进制不建窗口」不等于真正无头。两条路径：**A** 窗口从 `tauri.conf.json` 移到 `setup` 里条件创建 + `METATORIO_HEADLESS=1`，再加 `METATORIO_MCP=0` 开关（小改动，Linux 仍需 xvfb）；**B** 把 app 层胶水（上下文加载 + `execute_command`）抽成不依赖 Tauri 的共享 crate，新增 `metatorio-mcp` 二进制（真无显示服务器，但两套产物，与 mcp-design 决策 1 相悖）。
