...
- 求解失败要让 agent 看见：auto-plan 无解/无候选时只 `emit("solve-error")`，工具返回 `solve: null` + `changed: false`，agent 无法区分「无解」「参数无效」「什么都没做」。应返回结构化错误（或 MCP `isError`）。
