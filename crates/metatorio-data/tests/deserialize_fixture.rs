...
fn load_fixture() -> Value {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/prototypes-fixture.json");
...
fn assembling_machine_has_three_roles() {
    ...
    // 物品角色
    let item: ItemComponent = serde_json::from_value(item_json.clone()).expect("ItemComponent 反序列化失败");
    assert_eq!(item.stack_size, 50, "组装机可堆叠 50");

    // 配方角色
    let recipe: RecipeComponent = serde_json::from_value(recipe_json.clone()).expect("RecipeComponent 反序列化失败");
    assert!(recipe.ingredients.as_ref().is_some_and(|i| !i.is_empty()), "配方应有原料");
    // 2.0 的组装机配方 JSON 无 energy_required（走默认值），仅断言原料/产物

    // 制造机器角色（继承链上的 CraftingMachine 层）
    let machine: CraftingMachineComponent =
        serde_json::from_value(machine_json.clone()).expect("CraftingMachineComponent 反序列化失败");
    assert!((machine.crafting_speed - 0.5).abs() < 1e-9, "组装机1 速度 0.5");
    assert_eq!(
...
fn module_is_item_subclass() {
    ...
    let module: ModuleComponent = serde_json::from_value(module_json.clone()).expect("ModuleComponent 反序列化失败");
...
fn fluid_parses_temperature_and_heat_capacity() {
    ...
    let fluid: FluidComponent = serde_json::from_value(fluid_json.clone()).expect("FluidComponent 反序列化失败");
...
fn recipe_categories_and_results_parse() {
    ...
    let recipe: RecipeComponent = serde_json::from_value(recipe_json.clone()).expect("反序列化失败");
    assert!(recipe.ingredients.as_ref().is_some_and(|i| !i.is_empty()), "铁板配方应有原料");
    assert!(recipe.results.as_ref().is_some_and(|r| !r.is_empty()), "铁板配方应有产物");
...
fn ignored_visual_fields_do_not_break_parsing() {
    ...
    assert!((machine.crafting_speed - 0.75).abs() < 1e-9, "组装机2 速度 0.75");
}
