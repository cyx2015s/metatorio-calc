...
fn quality_formulas() {
    ...
    assert_eq!(q.default_multiplier(), Some(1.0));
    assert_eq!(q.accumulator_capacity_multiplier(), Some(1.0));
    assert_eq!(q.inventory_size_multiplier(), Some(1.0));
    assert_eq!(q.chain_probability(), Some(0.0));

    let mut q = QualityComponent::default();
    q.level = 3;
    assert_eq!(q.default_multiplier(), Some(1.9));
    assert_eq!(q.accumulator_capacity_multiplier(), Some(4.0));
    assert_eq!(q.electric_pole_wire_reach_bonus(), Some(6.0));
    assert_eq!(q.beacon_supply_area_distance_bonus(), Some(3.0));
    assert_eq!(q.range_multiplier(), Some(1.3));
    assert_eq!(q.tool_durability_multiplier(), Some(4.0));
    assert_eq!(q.locomotive_power_multiplier(), Some(1.03));

    // 字段有值时优先字段值
    let mut q = QualityComponent::default();
    q.level = 2;
    q.inventory_size_multiplier = Some(2.5);
    assert_eq!(q.inventory_size_multiplier(), Some(2.5));
    assert_eq!(q.cargo_wagon_inventory_size_multiplier(), Some(2.5));

    // 依赖链：cargo_wagon → inventory_size_multiplier → default_multiplier(1 + 0.3 * level)
    let mut q = QualityComponent::default();
    q.level = 2;
    assert_eq!(q.cargo_wagon_inventory_size_multiplier(), Some(1.6));
}

#[test]
fn entity_tile_size_from_collision_box() {
    let e = EntityComponent::default();
    assert_eq!(e.tile_width(), Some(0));
    assert_eq!(e.tile_height(), Some(0));

    let mut e = EntityComponent::default();
    e.collision_box = Some(BoundingBox(Vector(-1.0, -2.0), Vector(3.0, 4.0)));
    assert_eq!(e.tile_width(), Some(4));
    assert_eq!(e.tile_height(), Some(6));
...
#[test]
fn tile_effect_is_opaque() {
    let t = TileComponent::default();
    assert_eq!(t.effect_is_opaque(), Some(true));

    let mut t = TileComponent::default();
    t.effect_color = Some(Color(255, 255, 255, 128));
    assert_eq!(t.effect_is_opaque(), Some(false));
}
