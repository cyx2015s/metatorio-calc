    ...
    pub(crate) fn ui(&mut self, ui: &mut egui::Ui) {
            ...
            if self.style.enable_time_button {
                ui.menu_button("时间格式", |ui| {
                    ...
                    search_changed |= ui
                        ...
                        .changed();
                    search_changed |= ui
                        .radio_value(&mut self.style.time_format, TimeFormat::Hide, "隐藏")
                        .changed();

                    ui.separator();

                    search_changed |= ui
                        .radio_value(&mut self.style.time_precision, TimePrecision::Seconds, "秒")
                        .changed();
                    search_changed |= ui
                        .radio_value(
        ...
        ui.horizontal(|ui| {
            if self.style.enable_log_count {
                ui.label(format!("日志数量: {}", logger.logs.len()));
                ui.label(format!("显示数量: {}", logs_displayed));
            }
