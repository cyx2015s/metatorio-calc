...
use metatorio_core::prim_var::Variant;
...
fn item_recipe_expands_single_variable() {
    ...
    assert_eq!(v.prim_var.variant, Variant::Single);
    assert_eq!(
        v.flow.get(&DualVar::Item("iron-ore".into())),
        Some(&-1.0)
    );
    assert_eq!(
        v.flow.get(&DualVar::Item("iron-plate".into())),
        Some(&1.0)
    );
}
...
fn fluid_recipe_expands_two_interpolation_ends() {
    ...
    assert_eq!(low.prim_var.variant, Variant::Interp(0));
    assert_eq!(high.prim_var.variant, Variant::Interp(1));
    assert_eq!(low.prim_var.config, high.prim_var.config, "同配置共享 config 标识");

    // 两端共享：Fluid 流相同（水 -10、蒸汽 +10）
    for v in [low, high] {
        assert_eq!(v.flow.get(&DualVar::Fluid { name: "water".into() }), Some(&-10.0));
        assert_eq!(v.flow.get(&DualVar::Fluid { name: "steam".into() }), Some(&10.0));
        // 产物蒸汽 165°C：热量 = 10 × (165-100) × 1000 = 650 kJ
        assert_eq!(
            v.flow.get(&DualVar::FluidHeat { filter: "steam".into() }),
            Some(&650_000.0)
        );
    }
    ...
    assert_eq!(
        low.flow.get(&DualVar::FluidHeat { filter: "water".into() }).unwrap_or(&0.0),
        &0.0
    );
    // 高端（500°C 水）：热量 = 10 × (500-15) × 1000 = 4,850 kJ（消耗）
    assert_eq!(
        high.flow.get(&DualVar::FluidHeat { filter: "water".into() }),
        Some(&-4_850_000.0)
    );
}
...
fn expansion_is_order_independent() {
    ...
    let a = expand(&[recipe_mechanic("iron-plate"), recipe_mechanic("heat-water")], &ctx);
    let b = expand(&[recipe_mechanic("heat-water"), recipe_mechanic("iron-plate")], &ctx);
    ...
    assert_eq!(configs, vec![0, 0, 1], "heat-water=0（两个插值端共享），iron-plate=1");
    assert_eq!(a.variables[0].prim_var.config, a.variables[1].prim_var.config, "两个插值端共享 config");
}
