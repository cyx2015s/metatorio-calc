...
use metatorio_core::{DualVar, Flow, IdWithQuality, NORMAL_QUALITY};
...
fn planet_map_gen(store: &PrototypeStore, planet: &str) -> Option<PlanetPrototypeMapGenSettings> {
    store
...
pub fn planet_generated_tiles(store: &PrototypeStore, planet: &str) -> HashSet<String> {
    ...
    for record in store.group(PrototypeGroup::Tile) {
        ...
        if let Some(control) = &autoplace.control {
            if map_gen.autoplace_controls.contains_key(control) {
...
pub fn planet_autoplaced_flows(store: &PrototypeStore, planet: &str) -> Flow {
    ...
    for record in store.group(PrototypeGroup::Entity) {
        ...
        let Some(minable) = record.component::<EntityComponent>().and_then(|e| e.minable())
        else {
            continue;
        };
...
mod tests {
    ...
    fn seed_available_by_generated_tile_intersection() {
        ...
        assert!(seed_available_on_planet(&store, seed, "plant-entity", Some("nauvis")));
        // aquilo 只生成 shallow-water（water-tile）→ 不可种
        assert!(!seed_available_on_planet(&store, seed, "plant-entity", Some("aquilo")));
        // 无星球 → 视为可用
